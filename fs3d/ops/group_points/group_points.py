...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, points_xyz, center_xyz, features=None, seeds_xyz=None, seed_indices=None):
        # print("features_out: " + str(features.shape))
        """forward.
        ...
        if self.max_radius is None:
            ...
            # print("points_xyz: " + str(points_xyz.shape))
            # print("center_xyz: " + str(center_xyz.shape))
            # print("self.min_radius: " + str(self.min_radius))
            # print("self.max_radius: " + str(self.max_radius))
            # print("self.sample_num: " + str(self.sample_num))
            # print()
        ...
        if self.uniform_sample:
            ...
            for i_batch in range(idx.shape[0]):
                for i_region in range(idx.shape[1]):
                    ...
                    # print("num_unique: " + str(num_unique))
                    # print("sample_num: " + str(self.sample_num))
        ...
        if seeds_xyz != None:
            seeds_xyz_trans = seeds_xyz.transpose(1, 2).contiguous()
            seeds_xyz = grouping_operation(seeds_xyz_trans, idx)
            # print("seeds_xyz: " + str(seeds_xyz.shape))

        if seed_indices != None:
            seed_indices = seed_indices.unsqueeze(-1)
            seed_indices = seed_indices.transpose(1, 2).contiguous().float()
            seed_indices = grouping_operation(seed_indices, idx).long()
            seed_indices = seed_indices.squeeze(1)
        ...
        if len(ret) == 1:
            if seeds_xyz != None:
                return ret[0], seeds_xyz, seed_indices
            return ret[0]
        else:
            return tuple(ret)
