...
from fs3d.core import show_multi_modality_result, show_result
from fs3d.core.bbox import DepthInstance3DBoxes
...
class SUNRGBDDataset(Custom3DDataset):
    ...
    def path_reflash(self, path, sample_idx):

        path_split = path.split('/')
        path_split[-1] = str(sample_idx) + '.bin'

        return osp.join(path_split[0], path_split[-1])
    ...
    def get_data_info(self, index):
        ...
        info["pts_semantic_mask_path"] = self.path_reflash(info["pts_semantic_mask_path"], sample_idx)
        info["pts_instance_mask_path"] = self.path_reflash(info["pts_instance_mask_path"], sample_idx)

        if self.modality['use_lidar']:


            info['pts_path'] = self.path_reflash(info['pts_path'], sample_idx)
    ...
    def get_ann_info(self, index):
        ...
        # print(self.box_mode_3d)

        # print("self.box_mode_3d: " + str(type(self.box_mode_3d)))
        ...
        anns_results["pts_instance_mask_path"] = pts_instance_mask_path
    ...
    def evaluate(self,
        ...
        if isinstance(results[0], dict):
            return super().evaluate(results, metric, iou_thr, logger, show,
                                    out_dir, pipeline, few_shot_class=few_shot_class)
